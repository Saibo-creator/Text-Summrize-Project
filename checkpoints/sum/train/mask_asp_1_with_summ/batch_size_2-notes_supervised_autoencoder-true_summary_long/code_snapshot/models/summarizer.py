...
class Summarizer(object):
    ...
    def prepare_individual_revs(self, texts, append_edoc=False):
        ...
        if append_edoc:
            ...
            docs_ids, _, _,_ = self.dataset.prepare_batch(docs_reviews, dummy_ratings, global_append_id=EDOC_ID)
        else:
            docs_ids, _, _,_ = self.dataset.prepare_batch(docs_ids, dummy_ratings)  # [batch * n_docs, max_len]
    ...
    def run_epoch(self, data_iter, nbatches, epoch, split,
        ...
        for s, (hotel_url ,texts, ratings, metadata) in enumerate(data_iter):#data_iter: iterable providing minibatches(batch of unities, such as a group of two unity)
            #以batch 为单位itererate
            # texts: list of strs, each str is n_docs concatenated together with EDOC_TOK delimiter
            #print(metadata)={'item': ['hotel_review-g10006284-d151307-reviews-the_sands_at_grace_bay-grace_bay_providenciales_turks_and_caicos.html.json', 'hotel_review-g10006284-d151307-reviews-the_sands_at_grace_bay-grace_bay_providenciales_turks_and_caicos.html.json'],
            # 'short_summary': ['Excellent romantic hotel. Great rooms in excellent location.', 'Excellent romantic hotel. Great rooms in excellent location.'],
            # 'long_summary': ['Great location. Near to the city centre with good shopping options nearby.Good food, especially seafood. Acceptable restaurant prices, but poor overall dining experience.Nice hotel buildings and lovely terraces. Elevator in need of updating.Spacious, clean enough rooms with okay furnishing. Out-of-date, poorly maintained rooms with bad bathrooms and beds. Shower can be a problem.Service is welcoming but the reception is subpar. Service professionalism and housekeeping are average. Good service at bar and pool.Room cleanliness is acceptable. Clean hotel facilities.OK vibe and a friendly atmosphere. Hotel could use some updating. Stylish hotel. Nice for romantic trips. Awesome for a beach holiday. Awesome for families.Breakfast is average.', 'Great location. Near to the city centre with good shopping options nearby.Good food, especially seafood. Acceptable restaurant prices, but poor overall dining experience.Nice hotel buildings and lovely terraces. Elevator in need of updating.Spacious, clean enough rooms with okay furnishing. Out-of-date, poorly maintained rooms with bad bathrooms and beds. Shower can be a problem.Service is welcoming but the reception is subpar. Service professionalism and housekeeping are average. Good service at bar and pool.Room cleanliness is acceptable. Clean hotel facilities.OK vibe and a friendly atmosphere. Hotel could use some updating. Stylish hotel. Nice for romantic trips. Awesome for a beach holiday. Awesome for families.Breakfast is average.']}
            #print(len(hotel_url),len(texts),len(ratings))==(2,2,2)
            if s > nbatches:
                break
            ...
            if self.hp.concat_docs:#False
                if self.hp.true_summary:
                    docs_ids, _, labels,gold_summaries_ids = self.dataset.prepare_batch(texts, ratings, g_summ_loss=self.hp.gold_summ_loss, gold_summary_batch=metadata[self.hp.true_summary_type], doc_append_id=EDOC_ID)
                else:
                ...
                #in unsupervides mode, no gold summary is provided, only create a dummy variable
                gold_summaries_ids=None
            else:
                docs_ids = self.prepare_individual_revs(texts, append_edoc=True)
                if self.hp.true_summary:
                    _, _, labels,gold_summaries_ids = self.dataset.prepare_batch(texts, ratings, g_summ_loss=self.hp.gold_summ_loss, gold_summary_batch=metadata[self.hp.true_summary_type], doc_append_id=EDOC_ID)
                else:
            ...
            output = self.sum_model(docs_ids, labels,
                                    ...
                                    gold_summaries_ids=gold_summaries_ids,
                                    tau=cur_tau,
                                    adv_step=None,
            ...
            if s % self.opt.print_every_nbatches == 0:# self.opt.print_every_nbatches (default=1)
                ...
                if self.hp.true_summary:
                    try:
                        ...
                        src_docs = [[summary] for summary in metadata[self.hp.true_summary_type]] # it's src_docs when batch size >1  and src_doc when else
                        #hp.true_summary_type== 'short_summary' or 'long_summary'
                    ...
                    if self.hp.truncate_:
                        clean_summs=[re.match('.*?\.',summ).group(0) for summ in clean_summs]
                    avg_rouges, min_rouges, max_rouges, std_rouges = \
                ...
                if self.hp.truncate_:
                    raw_summ=summ_texts[0]
                    print('SUMMARY: ', re.match('.*?\.',raw_summ).group(0) )
                else:
                    print('SUMMARY: ', summ_texts[0].encode('utf8'))
                print('-' * 100, '\n')

                if self.hp.true_summary:
                    print('true_summary: ', metadata[self.hp.true_summary_type][0].encode('utf8'))
                    print('-' * 100, '\n')
