...
if __name__ == '__main__':
    ...
    parser.add_argument('--true_summ',default='',
                        help="set 'long' or 'short' if calculate rouge w.r.t  true summary")

    parser.add_argument('--g_summ_loss',default=False,
                        help="set True if want to calculate loss w.r.t gold summary(supervised baseline)")
    parser.add_argument('--truncate',default=False,
                        help="when test summarizer, set True to only select first sentence generated(helpful for highly repetitive case)")
    ...
    if opt.true_summ:
        ...
        if hp.true_summary=='long':
            hp.true_summary_type = 'long_summary'
        elif hp.true_summary=='short':
            hp.true_summary_type = 'short_summary'

    if opt.g_summ_loss:
        hp.gold_summ_loss = opt.g_summ_loss
    #TODO
